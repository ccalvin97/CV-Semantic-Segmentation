...
class network():
    ...
    def get_unet(self, pretrained_weights = None):
       ...
       # up7 = (UpSampling2D(size=(5, 5))(conv7))  # up7=25*25*3
        ...
        print(model.summary())
        model.compile(optimizer=Adam(lr=self.lr, decay=1e-6), loss=self.focal_loss(), metrics=[self.dice_coef, self.binary_accuracy, MeanIoU(num_classes=2), self.true_positive_rate])
